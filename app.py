...
PATTERN_LABEL = {'A': '型態A（漲後整理）', 'B': '型態B（多頭排列）', 'C': '型態C（回測後再噴）'}
...
with col_pattern:
    pattern_options = ['全部', '型態A（漲後整理）', '型態B（多頭排列）', '型態C（回測後再噴）']
...
# 套用篩選
pattern_filter_map = {'型態A（漲後整理）': 'A', '型態B（多頭排列）': 'B', '型態C（回測後再噴）': 'C'}
...
if selected_syms:
    ...
    b64 = base64.b64encode(tv_content.encode('utf-8')).decode()
    st.markdown(
        f'<a href="data:text/plain;base64,{b64}" download="{filename}" '
        f'style="display:inline-block;padding:0.4rem 0.8rem;background:#FF4B4B;color:white;'
